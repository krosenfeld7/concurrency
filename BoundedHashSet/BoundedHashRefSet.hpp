...
class BoundedHashRefSet {
private:
    DynamicCountingSemaphore boundedSemaphore;
    std::unordered_set<Entry> hashSet;
    std::mutex mutex;

public:
    BoundedHashRefSet(int bound)
     ...
     */
    auto contains(Entry& entry) -> bool {
        std::unique_lock<std::mutex> lock(mutex);
